...
render (CanvasState canvasCommands) =
  let canvas = foldl applyCommand Nothing canvasCommands
  in renderCanvasPixels canvas

applyCommand :: Maybe Canvas -> CanvasCommand -> Maybe Canvas
applyCommand canvas command =
  case command of
    CreateCanvas width height   -> Just $ Canvas width height IntMap.empty
    DrawLine point1 point2      -> fmap (drawLine point1 point2) canvas
    DrawRectangle point1 point2 -> fmap (drawRectangle point1 point2) canvas
...
renderCanvasPixels maybeCanvas =
  let makeRow canvas y = L.foldl (\xAcc x -> xAcc <> pointToText (x, y) canvas) "" [0 .. canvasWidth canvas + 1]
  in case maybeCanvas of
       Nothing     -> ""
       Just canvas -> intercalate "\n" $ L.map (makeRow canvas) [0 .. canvasHeight canvas + 1]
